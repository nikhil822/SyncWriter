    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        ...
        const password1FieldErrors = errors
          .filter((error) => error.param === "password1")
          .map((error) => error.msg);
        const password2FieldErrors = errors
          .filter((error) => error.param === "password2")
          .map((error) => error.msg);
      ...
      <div className="w-full max-w-sm bg-white dark:bg-slate-800 rounded border-primary shadow-md border dark:border-0 dark:shadow-xl p-6">
        <div className="flex flex-col space-y-4">
          ...
          <button
            onClick={register}
            disabled={loading}
            className="bg-blue-600 text-white text-sm font-semibold px-3 py-2 rounded hover:bg-blue-500 flex justify-center items-center space-x-1 active:ring-1 disabled:opacity-50 disabled:cursor-not-allowed"
          >
